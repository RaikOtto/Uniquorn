...
#' @title Identifiy cancer cell line
...
#' @usage \code{identify_vcf_file}( vcf_file = "./my_cl_vcf.vcf.gz", ref_gen = "GRCH37", mutational_weight_inclusion_threshold = 0.5, similarity_threshold = 10.0, manual_identifier_bed_file = c( "CL_a_CCLE", "CL_b_CUSTOM" ), write_xls = TRUE, batch_mode = FALSE, only_first_candidate = FALSE, output_file = "", distinct_mode = TRUE )
...
identify_vcf_file = function( 
    ...
    if ( output_file == ""  ){
        ...
        output_file = base::paste( output_file, base::paste( vcf_file_name, "uniquorn_ident.tab", sep ="_"), sep = "/") 
    ...
    base::print( base::paste0( c("Found ", base::as.character( base::length( unique(sim_list$CL) ) ), " many CLs for reference genome ", ref_gen ), collapse = "" ) )
    base::print("Finished reading database, identifying CL")
    
    # filter for weights
    if ( mutational_weight_inclusion_threshold != 0.0  ){
    
        base::print( base::paste0( c("Adjusted mutational inclusion weight, only using mutations that are have a weight higher than ", as.character(mutational_weight_inclusion_threshold)), collapse="") )
        
        sim_list = sim_list[ base::as.double(sim_list$Weight) >= as.double( mutational_weight_inclusion_threshold) ,  ]
