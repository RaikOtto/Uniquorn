...
'CCLE_hybrid_capture1650_hg19_NoCommonSNPs_CDS_2012.05.07.maf' from http://www.broadinstitute.org/ccle/data/browseData
...
```{r}
 add_custom_vcf_to_database( HT29_vcf_file, name_cl = "HT29_test_file", test_mode = TRUE )
```

Likewisely, if you want to remove the sample:

```{r}
    remove_custom_vcf_from_database( "HT29_test_file", test_mode = TRUE  )
```

If you want to see which CLs are contained:

```{r}
head( show_contained_cls( ref_gen = "GRCH37" ) )
```

If you want to know which mutations are overall contained in the training set for a particular database:

```{r}
 head( show_contained_mutations( ref_gen = "GRCH37" ) )
```

Same if you want to know which genomic loci are associated with a particular CL:

```{r}
head( show_contained_mutations_for_cl("SF_268_CELLMINER") )
```
